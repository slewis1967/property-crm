...
/**
 * CHARACTERIZATION tests for the Borrower Fact Find compliance-document routes
 * (list + POST create in ./route, GET one / PATCH / DELETE in ./[id]/route, and
 * the audit trail in ./[id]/history/route).
 *
 * These pin the CURRENT behavior before the shared-helper dedup refactor and
 * must keep passing UNCHANGED afterwards — that equivalence is the proof the
 * refactor preserved behavior. Supabase and requireAuth are mocked the way the
 * money-path route tests do (calculations/route.test.ts); the real
 * compliance-audit + factfind modules run through the mocked client.
 *
 * Behaviors pinned here: PII-safe LIST_COLUMNS (never `*`), the NARROW
 * table-missing check (a column error is NOT "table missing"), the exact table
 * name `borrower_fact_finds`, the locked-status (Complete) edit/delete refusals,
 * and the audit-hook invocation on create/update/delete.
 */
...
beforeEach(() => {
  ...
  h.state.insertResult = { data: { id: "ff-1" }, error: null };
...
describe("GET (list fact finds)", () => {
  it("selects the explicit PII-safe LIST_COLUMNS — never `*` — from borrower_fact_finds", async () => {
    h.state.selectResult = { data: [{ id: "ff-1", applicant_name: "Smith, John" }], error: null };
    const res = await listGet(jsonReq("GET"));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, factFinds: [{ id: "ff-1", applicant_name: "Smith, John" }] });
  ...
  it("degrades to an empty list when the TABLE is missing (PGRST205)", async () => {
    ...
    const res = await listGet(jsonReq("GET"));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, factFinds: [] });
  });
...
describe("POST (create a fact find)", () => {
  it("inserts denormalized columns from the data blob, defaults status Draft, and records a create audit", async () => {
    const data = {
      ...
      referred_by: "Referrer Co",
    };
    const res = await POST(jsonReq("POST", { data }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, id: "ff-1" });
    ...
    expect(row.referred_by).toBe("Referrer Co");
    ...
    expect(a.doc_type).toBe("fact_find");
  ...
  it("returns the MIGRATION_HINT with 501 when the table is missing on insert", async () => {
    ...
    expect(((await res.json()) as { error: string }).error).toContain("Fact Find storage isn't set up");
  });
});

describe("GET (one fact find)", () => {
  it("returns the hydrated document from borrower_fact_finds", async () => {
    h.state.selectResult = { data: { id: "ff-9", status: "Draft", data: {} }, error: null };
    const res = await oneGet(jsonReq("GET"), params("ff-9"));
    expect(res.status).toBe(200);
    const body = (await res.json()) as { ok: boolean; factFind: { id: string; data: unknown } };
    expect(body.ok).toBe(true);
    expect(body.factFind.id).toBe("ff-9");
    // hydrateFactFind expands the blob to the full template shape.
    expect(body.factFind.data).toHaveProperty("applicants");
...
describe("PATCH (save a fact find)", () => {
  it("updates a draft, re-derives denormalized columns, and records an update audit", async () => {
    ...
    const data = {
      applicants: [
        { family_name: "Doe", given_names: "Jane" },
        { family_name: "", given_names: "" },
      ],
      loan: { amount_required: 250000 },
      referred_by: "Ref2",
    };
    const res = await PATCH(jsonReq("PATCH", { data }), params("ff-1"));
    expect(res.status).toBe(200);
    ...
    expect(patch.referred_by).toBe("Ref2");
...
describe("DELETE (remove a fact find)", () => {
  it("deletes an unlocked document and records a delete audit", async () => {
    h.state.selectResult = { data: { status: "Draft", data: { referred_by: "x" } }, error: null };
    const res = await DELETE(jsonReq("DELETE"), params("ff-1"));
    expect(res.status).toBe(200);
...
describe("GET (fact find history)", () => {
  it("returns the audit trail rows", async () => {
    const rows = [{ id: "a1", doc_type: "fact_find", action: "create" }];
    h.state.historyResult = { data: rows, error: null };
    const res = await historyGet(jsonReq("GET"), params("ff-1"));
    expect(res.status).toBe(200);
