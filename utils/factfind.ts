...
/* ── Supabase plumbing (mirrors utils/deals.ts) ──────────────────────────── */
 ...
 *
 * Narrower than the equivalent in utils/deals.ts on purpose. A missing *column*
 * reports as PGRST204 ("Could not find the 'x' column … in the schema cache")
 * or 42703 ("column … does not exist") — both of which a loose `schema cache` /
 * `does not exist` substring test swallows, telling the operator to run a
 * migration they have already run. Match the table-level codes, and only fall
 * back to a substring test that names a *table*.
 */
