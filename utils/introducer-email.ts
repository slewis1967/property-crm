...
export async function sendNewPackNotice(opts: {
  ...
  const needsAnalysisUrl = `${portalBaseUrl()}/needs-analysis/${opts.needsAnalysisId}`;
  ...
  return sendBrevoEmail({
    ...
    html: internalShell(
      ...
      `<p><strong>${opts.firmName}</strong> (${opts.introducerName}) has submitted a full pack.</p>
       ...
       <p style="color:#6b7280;font-size:13px;">This did NOT go through the review queue. A Tier 2 pack arrives with a completed Needs Analysis, so the opportunity${
         opts.opportunityId ? ` (${opts.opportunityId})` : ""
       } and the Needs Analysis were created on submission and it is with the assessor now.</p>
       ${documents}
       <p style="margin:24px 0;">
         ...
         <a href="${needsAnalysisUrl}" style="color:#1b1f44;text-decoration:underline;padding:12px 8px;display:inline-block;">Open the Needs Analysis</a>
       </p>`,
