...
### Channel Partners (`app/channel-partners/` + `app/api/channel-partners/` + `utils/channel-partners*.ts`, `utils/channel-partner-contacts.ts`)
Recruiting firms that sell new-build stock (property marketers, building brokers, investor clubs, SMSF / SDA / multi-tenancy specialists) to take NextKey-sourced stock. Sidebar link **Channel Partners** under CRM. Table `channel_partners` (`migrations/20260911_channel_partners.sql`, applied; seeded with the 24-firm research list). Two tabs: **Partners** (list + a drawer per firm: contact check, pipeline fields, tailored pitch email + phone script) and **Pitch deck** (the 5-page A4 brochure, tailorable per partner; `/channel-partners/brochure?partner=<id>` prints it to PDF).
- **Streams drive the pitch.** `parseStreams()` maps the free-text `nextkey_stream_fit` to `core | smsf | sda | multi | springboard`; `buildPitchEmail()` / `buildCallScript()` (pure, tested) assemble copy per stream. The copy was compliance-reviewed on 2026-09-11 — the file header in `utils/channel-partners.ts` lists the rules each phrase encodes (no yield/rent/occupancy, LRBA-ban accuracy, "under your brand", coarse round-DOWN stock figures with an as-at date). Change wording only with the same care; `utils/channel-partners.test.ts` pins the load-bearing phrases.
- **Stock figures are live, never typed.** `getStockStats()` counts `global_stock_pool` rows whose status contains "available" (paged, 10-min cache); `niceCount()` rounds DOWN coarsely. A pitch with no usable feed quotes no figures rather than failing.
- **Springboard is gated mechanically (clause 7.5 — per piece).** The Springboard paragraph and brochure page 5 only appear when `CHANNEL_PITCH_SPRINGBOARD_REF` is set in the env (record it only once YLA's WRITTEN approval of this exact text is filed and `check_clause7_evidence.py` passes). Until then page 5 renders under a NOT APPROVED wash and only in internal draft previews. **The generic Springboard home-buyer copy and SMSF copy never share a document** — the email drops it for SMSF-stream partners, and a brochure carrying page 5 drops the SMSF tile.
- **SMSF firms get their own, separately approved LRBA-ban paragraph** (`SMSF_YLA_PARAGRAPH` + `SMSF_YLA_SCRIPT_LINE`, env `CHANNEL_PITCH_SMSF_REF` — a different clause 7 piece, so a different variable; the Springboard ref never switches it on). Wording is "Option B" from the 2026-09-11 compliance review: it says what the purchase IS (personal, own name, home loan + separate deposit loan, fees apply) rather than a denial-only line about what the SMSF doesn't do (flagged as a half-truth), never explains the super mechanic, never says "workaround". The review's Option A (a sentence tying a client's super to eligibility) is RULED OUT by Sean, 2026-09-12 — never add it, whatever YLA offers; a test pins it. The YLA request (sent 2026-09-11, Option A withdrawn 2026-09-12) still needs their written answers on Chapter 7 / SIS / TMD / which entity may refer — don't set the ref before those exist.
- **Sending reuses `/api/emails`** (email_log row, sender's own signature, tag `channel-partner`), then `PATCH {action:"pitched"}` stamps `pitch_sent_at`/`last_contacted` and moves "Not contacted" → "Contacted" (never demotes a later status). The panel refuses to send when: no email, status is Signed/Not a fit/Do not contact, the address is in `unsubscribes`, the contact isn't verified within 90 days, or the exact text hasn't been through `reviewBroadcastCopy` (violations need an explicit override tick).
- **Contact freshness.** `contact_verified_at` + a 90-day window (`contactFreshness`). `POST /api/channel-partners/[id]/verify {action:"check"}` reads the firm's own site (home + contact/about/team pages, manual redirect-following through `isPubliclyRoutable` — the SSRF guard exported from `utils/lender-policy/source-check.ts`) and runs a `MODELS.fast` `:online` lookup in parallel (~5–20s, inside Netlify's ceiling). **A detail is only "confirmed" when that exact email/phone appears on the firm's own site** — AI suggestions stay "unconfirmed" and are never pre-ticked; Cloudflare-protected emails are decoded; phones must match real AU groupings (a loose pattern once matched an ID). A check changes no contact field; the only automatic stamp is "verified" when every detail we hold is still published unchanged. `{action:"apply"}` writes ticked proposals + stamps verified; `{action:"manual", note}` records "confirmed another way".
- **Scheduled re-check:** cron job `partners` (`/api/cron/run?job=partners`, deliberately NOT in `all` — one check can take ~20s) via `.github/workflows/partner-contact-checks.yml`, three calls daily at 06:30 AEST, one stale partner per call, never re-checking a partner more than weekly while a result awaits review.
...
| `/channel-partners` | **Channel Partners** — recruit firms to take NextKey-sourced stock: target list, contact freshness + website check, tailored pitch email / phone script, pitch-deck brochure. See *Channel Partners* in ARCHITECTURE |
| `/channel-partners/brochure` | The brochure alone for Print → PDF. `?partner=<id>` tailors it; `?springboard=draft` forces the (unapproved) Springboard page in for internal review |
