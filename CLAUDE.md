...
- **SMSF firms get their own, separately approved LRBA-ban paragraph** (`SMSF_YLA_PARAGRAPH` + `SMSF_YLA_SCRIPT_LINE`, env `CHANNEL_PITCH_SMSF_REF` — a different clause 7 piece, so a different variable; the Springboard ref never switches it on). Wording is "Option B" from the 2026-09-11 compliance review: it says what the purchase IS (personal, own name, home loan + separate deposit loan, fees apply) rather than a denial-only line about what the SMSF doesn't do (flagged as a half-truth), never explains the super mechanic, never says "workaround". The approval request to YLA asks them to choose Option A/B and to answer the open Chapter 7 / SIS / TMD questions — don't set the ref before those written answers exist.
