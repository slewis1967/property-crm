...
- **Two sign-in hardenings the introducer copy does NOT have yet** (security review 2026-09-11): (1) `redeemCode` spends the attempt with a compare-and-set (`.eq("attempts", read)`) BEFORE comparing — read-compare-increment let N parallel guesses share one attempt, so "5 attempts" was really "5 rounds". (2) The emailed link opens `/partner/verify`, a page with a **Sign in** button that POSTs the token; `GET /api/partner/verify` never redeems (it forwards to the page). Mail scanners (Mimecast/Proofpoint/Defender) open links on delivery — a redeeming GET let them burn link+code (one row) and hold a live session. Port both to `utils/introducer-auth.ts` when that portal is next touched.
...
- **Public pages get no staff sidebar at all** (applies to every public surface, not just this one): `proxy.ts` sets the `x-nk-surface: public` request header for `isPublicSurface` paths — and strips any client-sent value on every path — and `app/layout.tsx` then skips `getSidebarCounts()` and the `<Sidebar>` element. AppShell already *hid* the sidebar on these pages, but a hidden server-rendered prop is still serialised into the page payload, so `/introducer`, `/sign`, `/book` etc. were handing outsiders our queue counts. See `utils/public-surface.ts`.
