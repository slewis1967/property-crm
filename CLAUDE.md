...
- **Springboard is gated mechanically (clause 7.5 — per piece).** The Springboard paragraph and brochure page 5 only appear when `CHANNEL_PITCH_SPRINGBOARD_REF` is set in the env (record it only once YLA's WRITTEN approval of this exact text is filed and `check_clause7_evidence.py` passes). Until then page 5 renders under a NOT APPROVED wash and only in internal draft previews. **Springboard and SMSF never share a document** — the email drops Springboard for SMSF-stream partners, and a brochure carrying page 5 drops the SMSF tile.
