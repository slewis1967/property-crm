...
- **Still Google:** the opportunity page's **"Self-book link"** dropdown points to hosts' Google Calendar booking pages (`SchedulingHost.bookingPageUrl`). That's an external client-self-service capability with no in-house replacement — deliberately left intact, NOT part of the in-CRM meeting flow.
