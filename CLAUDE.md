...
- **Exam waiting periods can be waived, per invitation** (`{ waive: true }` on `POST .../exam-invite`, super-admin only). The reading timer, the cooling-off ladder and the 24h five-failure lock all live in the **course's** JavaScript (`03-course/index.html` in the Springboard pack), enforced against state on the candidate's device — so the waiver rides inside the HMAC-signed invite payload, where the candidate cannot forge it, and the exam function reads it off the signed token rather than the request body. It is never silent: the course shows a banner, the audit file gets `exam_invite_waiting_periods_waived` and a waived pass gets `exam_passed_with_waiting_periods_waived`. Action NAMES carry it because the audit file renders the action and nothing else. **Both repos must change together** — `mintExamInvite` here and `verifyInvite` in the pack's `netlify/functions/exam.mjs`.
