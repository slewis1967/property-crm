...
- `utils/paid-service-alerts.ts` — the sweep. ONE digest per Brisbane day listing everything (not one email per problem), silent when nothing is wrong, and `info`-severity items never email. Automatic sends are held outside **07:00–19:00 Brisbane** (`withinSendWindow`): `job=all` on the 2-hourly sweep also runs this job and the Brisbane day rolls over at 14:00 UTC, so without the window a pre-dawn sweep claims the day's one digest and the email arrives at 05:39. Holding costs nothing — the next in-window run sends it. Gated on `PAID_ALERTS_ENABLED=true`; recipient is env-fixed (`PAID_ALERTS_TO`), never request-supplied. Driven by `POST /api/cron/run?job=accounts` from `.github/workflows/paid-account-alerts.yml` (daily 21:00 UTC = 07:00 Brisbane), separate from `crm-sweeps.yml` so it can't queue behind the AI-heavy client sweeps.
...
- **Tipping-off is enforced SERVER-side, not just flagged.** Disclosing that an SMR exists is a criminal offence, so a UI badge is not a control — the API would hand the row to anyone. `canViewConfidentialReport()` (utils/aml.ts) is an ALLOW-list: the appointed compliance officer, anyone in `program.smrAccess`, and the person who lodged it (they formed the suspicion). It **fails closed** — an unconfigured program exposes nothing to anyone but the lodger. `GET /api/aml/reports` filters confidential rows and returns a `withheld` count; every verb on `reports/[id]` runs `guardConfidential`, which answers **404 rather than 403** because "you may not see this SMR" and "this SMR exists" are the same disclosure. Manage the list on `/aml/program`. **Lodged reports are immutable** (no edit/delete) for record keeping.
- **Ongoing CDD** (`migrations/20260730_aml_ongoing_cdd.sql`): CDD is not a one-off gate at onboarding — sanctions and PEP lists change after you onboard someone. `aml_cases.next_review_at` / `.last_reviewed_at` drive a risk-based cadence (`REVIEW_CADENCE_MONTHS`: high 6mo / medium 12 / low 24), stamped on create and **re-derived whenever the risk rating changes**, so escalating a case pulls its review forward. `isScreeningStale()` treats never-screened as stale. Existing rows are deliberately NOT backfilled — inventing review dates would manufacture a compliance record nobody performed.
- **Source of wealth** is captured separately from source of *funds* (funds can be a clean transfer while the wealth behind them is not) and is required by `cddCompleteness` **only when `needsEnhancedDd()` is true** — demanding it on every low-risk buyer trains people to type anything in the box.
- **Program obligations** — `programObligationsOutstanding(program, today)` is the single definition of "what's outstanding" (enrolment, officer, approval, independent evaluation incl. un-remediated findings, overdue/unreferenced compliance report) and drives the amber banner on `/aml/program`. Never-recorded counts as outstanding: an obligation with no evidence is not satisfied because nobody wrote a date down. The **independent evaluation** block captures evaluator/scope/findings/remediation, not just a due date; the **AUSTRAC compliance report** lives on the program, NOT in `REPORT_TYPES` — it's a periodic return about the entity, so business-day deadline maths would be meaningless.
- **Employee due diligence** rides on `aml_training` (`edd_completed_on`/`edd_outcome`/`edd_notes`, `migrations/20260730_aml_staff_due_diligence.sql`) rather than a second table — same subject, same evidence trail, and two registers that must be kept in step is how one goes stale. The route falls back to the base columns if that migration hasn't run.
...
### Introducer portal (`app/introducer/` + `app/api/introducer/`; staff side `app/admin/introducers/` + `app/api/admin/introducers/`; `utils/introducer*.ts` + `utils/super-admin.ts`)
External introducer firms register YLA clients and track their progress, segregated to their own clients. Sidebar link **Introducers** under CRM. Migration `migrations/20260811_introducer_portal.sql`.
- **Three subtrees, and the names are load-bearing.** The PUBLIC portal owns `/introducer/*` + `/api/introducer/*`, exempted from the Cloudflare Access gate by `isPublicIntroducerRoute` in `proxy.ts` (needs a matching CF Access **bypass** app). The STAFF side lives at `/admin/introducers` + `/api/admin/introducers/*` — a different subtree entirely, so it keeps its CF Access header no matter how the carve-out is edited later. **`"/introducers".startsWith("/introducer")` is true**, so the exemption matches `"/introducer"` exactly plus `"/introducer/"`, never a bare prefix. Same rule in `AppShell` and `PublicRouteGate`. Pinned by tests in `utils/introducer.test.ts`.
- **The introducer-facing half is SPRINGBOARD-branded, the staff half is NextKey.** An introducer refers clients to the Springboard product and their client will only ever hear the name Springboard; NextKey is the internal CRM and means nothing to them. So `app/introducer/*` and every email in `utils/introducer-email.ts` use the Springboard logo (`/api/portal/logo` — that prefix already has the CF Access bypass, which is why an emailed logo renders in Gmail at all), navy `#020e40` / amber `#c7894e`, and the `hello@springboardhomes.com.au` verified Brevo sender (same identity as `utils/mailIdentities.ts`). `/admin/introducers` and the internal new-referral notice (`internalShell`) deliberately stay NextKey CRM — dressing an internal alert as Springboard makes the one email that tells us to act look like the ones we send out. A test in `utils/introducer.test.ts` fails if "NextKey" reappears in the consent statement or any refusal message an introducer can see.
- **Introducers never hold a Cloudflare Access identity** — that would put them one over-broad CF policy away from the whole CRM. `utils/introducer-auth.ts` runs its own session: a one-time 6-digit code plus an emailed link (both single-use, only SHA-256 hashes stored, attempt-capped and DB-throttled), then a session token in an httpOnly `__Host-` cookie, again hash-only in `introducer_sessions`. Every request re-checks user AND firm status, so suspending either locks them out now rather than when the cookie expires. The emailed link lands on `/introducer/verify`, a confirm page that POSTs, and the code's attempt counter is spent with a compare-and-set — the two hardenings described under *Channel-partner portal*, which both portals now share.
- **Segregation has exactly one implementation.** `app/api/introducer/_shared.ts` exposes `loadOwnClient`/`listOwnClients`, which filter on the introducer id from the session. There is deliberately NO helper that loads a referral by id alone, so "forgot the tenant filter" isn't a mistake a route in that directory can make. "Not yours" and "doesn't exist" both 404.
- **Submit = lock, enforced twice.** `evaluateEdit` (pure, `utils/introducer.ts`) is the app-side gate; `introducer_clients_locked_guard` is a database trigger that rejects the same writes independently. Three ways through and no others: it's still a draft; a **super-admin unlock grant** covers every changed field (single-use, time-boxed, reason mandatory); or an **open info request** named a field that is blank and is being filled — additive only, never an overwrite. Adding a field to `INTRODUCER_FIELDS` without adding it to the trigger's column list would make that field editable after submit without authorisation.
- **One authority.** `utils/super-admin.ts` (`SUPER_ADMIN_EMAILS`, default Sean) gates accept/decline and unlock grants; ordinary staff can review, request more information, and move the progress stage. Env-configured rather than a table — a table of admins is a table someone can be added to. Fails closed on the `cf-access` unauthenticated sentinel.
- **Nothing reaches the pipeline until accepted.** Referrals live in `introducer_clients` until `POST .../decision {action:"accept"}` creates the NEXUS opportunity (the same `nexusApi("/api/leads")` call as lead promote, tagged `source: "introducer:<firm>"`). If NEXUS is unreachable the referral is NOT marked accepted — one claiming "accepted" with no opportunity behind it is worse than one still visibly in the queue.
- **What an introducer sees is a whitelist, not a redaction**: `toPortalView` plus the fixed `INTRODUCER_STAGES` milestone list, plus `message_to_introducer` (the only free text travelling back). A new internal column is invisible to the portal by default. `introducer_events` is append-only — a trigger rejects UPDATE/DELETE — because "nothing changed without authorisation" is only worth something if the evidence can't be edited.
- Documents ride on the private `introducer-documents` bucket (signed URLs, service-role only) and can ONLY be uploaded against an open info request that named that label. `introducerTablesMissing()` matches the table-level codes only (`42P01`/`PGRST205`), like `factFindsTableMissing()`; a missing migration is silent from both ends here, so `beginLogin()` logs it loudly.
- **NDA e-signing, step one of onboarding** (`utils/introducer-nda.ts` + `app/api/introducer/onboarding/[token]/nda` + `SignNda.tsx`). The applicant clicks **Read and sign the agreement**, which issues the `introducer_nda` document on first use and mints a signing link into the existing engine at `/sign/<token>`. Signing completes → `markDocSigned` → a hook in `app/api/sign/[token]/complete` advances the application `invited → nda_signed`, which is what opens the ID step. That hook is **best-effort by design**: the signature is already durably recorded, so failing it must not 500 a completed signing and invite a second one — it logs, and `POST .../nda` self-heals if it finds the document already signed.
  - **Every roadmap state that asks the applicant for something MUST render a section on the page.** A roadmap entry with nothing beneath it is a dead end — `invited` shipped that way and the first two invitees hit it, told to sign an agreement with no means to do so.
  - **Exactly ONE `signature_requests` row per signer, rotated in place.** Not tidiness: the completion route calls `allSigned(rows)`, which selects *every* row for the document and requires all of them to be `signed`. Inserting a fresh request per visit and marking the old one `expired` leaves a permanently-unsigned row, so `allSigned` never becomes true, `markDocSigned` never runs, and the document sits `Draft` while the signer is told it worked. Re-opening rotates `token_hash` on the one row, which kills the old link and keeps the invariant.
  - `ndaDataFor()` is pure and snapshots the applicant into the document. `readyToIssue()` exempts the NDA from needing an accreditation number — it is signed before the exam, so there isn't one yet.
- **Accreditation expires, and the expiry is enforced** (`migrations/20260819_introducer_accreditation_expiry.sql`). Twelve months for the accreditation, **six for the SMSF competency** — two clocks, because M4 is 23% of the Tier 1 exam and goes stale fastest. Both are derived once at certificate issue by `accreditationExpiries()` and stored as `date`, never recomputed at read time; a **reissued** certificate derives from the FIRST issue date, so replacing a misspelt name does not hand out another year. `addMonthsClamped` clamps to month end (31 Aug + 6 = 28 Feb) because JS date arithmetic rolls over. The dates are copied onto the firm at activation and resolved with the session (`IntroducerIdentity.accreditationExpiresAt`), so `POST .../submit` refuses an expired accreditation without a second query. **NULL is "not recorded", never "expired"** — every firm activated before this shipped has no dates, and reading NULL as expired would have shut the portal for all of them. Only the accreditation blocks submission; a lapsed SMSF competency is reported and narrows what may be discussed.
- **The consent statement names the whole disclosure chain**, not just Springboard: G.B. Mayes Holdings → CRE8 Finance t/a Your Loan Assist → a licensed financial adviser → lenders, plus the two conflict disclosures (independent, not an agent; may be paid a fee). Consent to pass details to one company does not cover onward disclosure to a credit licensee and a separate advice business. Pinned field-by-field in `utils/introducer.test.ts`; it is stored verbatim on the row, so changing it never applies retrospectively.
- **Submitting needs the signed form, not just the tick.** `CONSENT_FORM_LABEL` documents are the one upload that is NOT a reply to an information request — allowed on a **draft** only, `info_request_id` null — and `submit` refuses without one. Once submitted, a replacement needs an information request like every other document.
- **The course's gates can be overridden, per invitation** (`{ override: true }` on `POST .../exam-invite`; *Reopen the course for them* on the accreditation card). **Gated on `canOverrideCourse`, NOT on super-admin** — `COURSE_OVERRIDE_EMAILS`, defaulting to the owner alone. Deliberately its own variable: `SUPER_ADMIN_EMAILS` is a list that grows for good operational reasons, and a colleague who can accept referrals and authorise unlocks must not thereby be able to soften what accreditation requires. Checked in the route, not just hidden in the UI. Four things at once: the reading timer, the cooling-off ladder, the 24h five-failure lock **and its progress wipe**, and the re-locking of any module the candidate has already attempted — which is the case it exists for, a failed exam sending someone back through modules they had passed. It also **keeps their progress**, where every other re-issued link deliberately clears it; `resetIfReissued` skips the wipe for an override token, because wiping first would leave nothing to reopen. A module never opened stays locked (`attempts > 0` is the test) and the pass mark is unchanged — it unsticks, it does not shorten. All of it lives in the **course's** JavaScript (`03-course/index.html` in the Springboard pack), enforced against state on the candidate's device, so the flag rides inside the HMAC-signed invite payload where the candidate cannot forge it, and the exam function reads it off the signed token rather than the request body. Never silent: the course banners it, the audit file gets `exam_invite_waits_and_gates_overridden`, and a pass sat that way gets `exam_passed_under_override`. Action NAMES carry it because the audit file renders the action and nothing else. **Both repos must change together** — `mintExamInvite` here and `verifyInvite` in the pack's `netlify/functions/exam.mjs`.

### Channel-partner portal (`app/partner/` + `app/api/partner/`; staff side `app/admin/partners/` + `app/api/admin/partners/`; `utils/partner*.ts`)
External firms that SELL our stock (property marketers, investor clubs, advisers — the `/channel-partners` recruitment list) log in at `/partner`, browse the whole stock book, register their buyers and request holds. **Every firm sees all stock; each firm sees only its own clients and deals.** Sidebar link **Partners** under CRM. Migration `migrations/20260911_partner_portal.sql`. Built on the introducer portal's pattern — read that section first; the rules below are the differences.
- **Sean's decisions (2026-09-11), don't re-litigate:** built inside the CRM, not in PropChannel OS; **stock is masked and NextKey reveals the supplier per deal**; the partner sees **their referral fee per lot, never the gross fee or our share**; white-label and anything beyond basic is **sold on request and switched on by staff** — partners have no route that writes branding or plan.
- **Auth = the introducer model on its own tables** (`utils/partner-auth.ts`): emailed one-time link + 6-digit code, hash-only storage, `__Host-nk_partner` cookie, user AND firm status re-checked on every request. Tier, features and branding are resolved from the firm row inside `resolveSession`, never from the caller. Deliberately a copy of `introducer-auth.ts` rather than a shared module — parameterising a live login path wasn't worth the saving; `partner-auth.test.ts` pins the properties.
- **Two sign-in hardenings, now in BOTH portals** (security review 2026-09-11; ported to the introducer portal 2026-09-12): (1) `redeemCode` spends the attempt with a compare-and-set (`.eq("attempts", read)`) BEFORE comparing — read-compare-increment let N parallel guesses share one attempt, so "5 attempts" was really "5 rounds". (2) The emailed link opens `/partner/verify` (`/introducer/verify`), a page with a **Sign in** button that POSTs the token; the `GET` never redeems, it forwards to that page, so links already in inboxes keep working. Mail scanners (Mimecast/Proofpoint/Defender) open links on delivery — a redeeming GET let them burn link+code (one row) and hold a live session. **Keep the two copies in step**: `utils/partner-auth.test.ts` and `utils/introducer-auth.test.ts` pin them.
- **Carve-out:** `isPublicPartnerRoute` in `proxy.ts` (needs a matching CF Access **bypass** app for `/partner` + `/api/partner`). `"/partners".startsWith("/partner")` is true, so it matches `/partner` exactly plus `/partner/` — never `/admin/partners`, `/api/admin/partners` or `/channel-partners`. Same test in `AppShell`/`PublicRouteGate` via `isPartnerPortalPath`. Pinned in `utils/partner.test.ts`.
- **Segregation has one implementation**, `app/api/partner/_shared.ts` (`loadOwnClient`/`listOwnClients`/`loadOwnDeal`/`listOwnDeals`), all filtered on the session's `partnerId`. And the database backs it: `partner_enquiries (client_id, partner_id)` is a **composite FK** to `partner_clients (id, partner_id)`, so a deal can't reference another firm's buyer even from a buggy handler. What goes back is a whitelist (`toClientView`, `toDealView`).
- **Stock masking = `toPartnerLot` (utils/partner.ts), a whitelist.** Never included: builder, estate, lot number, street address, brochure/floorplan/image URLs (builder-branded), description/inclusions/rebates/specs (free text that names the estate or design range), raw status, source fields. `utils/partner-stock.ts` doesn't even SELECT them for a partner read. The short free-text fields that DO go (completion, land registration, property type) pass through `redactSupplierNames` against every builder/estate name we know (cached 10 min). `assertNoForbiddenKeys` is the tripwire on every payload and throws — a 500 beats a leak. **No images in v1 for exactly this reason** (renders/brochures carry builder logos).
- **Reveal is a separate staff act** (`action:"reveal"` on `/api/admin/partners/deals/[id]`), never a side effect of granting a hold. It writes `revealed` on that one deal and goes to the portal as label/value pairs (`revealedForPortal`), so the key-tripwire doesn't need a whitelist hole. Logged to `partner_events` (append-only trigger).
- **Fees:** `referralFee(gross)` = gross − max($10k, 30% of gross), null if nothing is left — ported from PropChannel OS. `PLATFORM_FLOOR_MIN` ($10k) can't be configured lower; `PARTNER_FEE_FLOOR` / `PARTNER_FEE_PERCENT` env can raise/tune it. Gross comes from `property_financials.gross_developer_fee` (edited on `/properties/[id]`); **a lot with no fee shows "Ask us"**. Only the partner's figure is snapshotted onto a deal (`referral_fee_snapshot`, re-snapshotted at hold grant); gross and our share are never stored in a partner table.
- **Availability** is derived from the free-text `status` by `lotAvailability` (sold/settled/under contract → unavailable; "hold" or an active partner deal → on hold). **One active hold per lot** is a partial unique index (`partner_enquiries_one_active_hold`); stage moves are conditional on the stage read (`.eq("stage", from)`), so two tabs can't both act. Holds don't lapse automatically — the staff queue flags `hold_expired`.
- **Hold granted → NEXUS opportunity** (`source: partner:<firm>`), best-effort AFTER the hold, since the hold is the business fact; failure is logged and there's a *Create* retry button. NEXUS requires an email, so phone-only clients get no opportunity.
- **Tiers** (`PARTNER_TIERS`/`TIER_DEFINITIONS`): basic (stock, clients, deals, fee; 2 logins), professional (5 logins), enterprise (unlimited + white-label). `feature_grants` adds extras per firm. Features with `built:false` are listed on the Plan page as "coming soon" and never usable. **Prices aren't in code** — they're Sean's, per agreement. Login caps are enforced when staff add a login.
- **White-label** = `partners.branding` {display_name, logo_url (https only), primary_color/accent_color (#hex only)}, validated by `parseBranding` so values can go into style attributes safely, applied by `resolveBranding` only while the firm holds `white_label`. Emails stay NextKey-branded (the recipients are the partner's staff, and the sender must be Brevo-validated); the sign-in page is NextKey because nobody is signed in yet.
- **Public pages get no staff sidebar at all**: the portal is added to the `isPublic` list in `proxy.ts`, which tags the request `x-public-route: 1` (stripping any client-sent value first) so `app/layout.tsx` skips `getSidebarCounts()` and the `<Sidebar>` entirely — AppShell only *hides* it, and a hidden server-rendered prop is still serialised into the page payload. That mechanism arrived in PR #265; see `docs/SECURITY-public-routes.md`, and keep the list there in step with the carve-outs.
- Staff queue emails `PARTNER_NOTIFY_EMAILS` (default Sean) on every hold request, naming the supplier — the one place outside the CRM that happens, deliberately.

### Property shortlists — send a client selected stock (`app/shortlist/` + `app/api/shortlist/`; staff side `app/api/property-shortlists/`; `utils/property-shortlist*.ts`)
Staff tick one or many lots in the Aggregator Feed and press **📨 Send to client** (`SendShortlistModal`). The client gets a private link `/shortlist/<token>` with five tabs — Properties, Compare, Costs & cashflow, Suburbs, Talk to us — marks each property *Interested* / *Not for me* with a note, and can book a call (`/book/<slug>`). Responses show in `ContactShortlists` on the contact page and email the consultant + `SHORTLIST_NOTIFY_EMAILS` (default Sean). Migration `migrations/20260914_property_shortlists.sql`.
- **Sean's decisions (2026-09-14), don't re-litigate:** NextKey branded (portal + email); builder, estate, lot number and street address hidden; reports = comparison, yield & cashflow, suburb profile, stamp duty & upfront costs; client can register interest and book a call.
- **Token = the document-portal model**: 256-bit raw token in the link only, SHA-256 in `property_shortlists.token_hash`, every public handler re-resolves it (`resolveShortlistToken`) and scopes item reads to that shortlist. Unknown, revoked and expired all return the same 404 message. There is no re-issue — a lost link means a new shortlist. Staff can revoke.
- **Carve-out:** `isPublicShortlistRoute` in `proxy.ts` — `/shortlist` exactly plus `/shortlist/`, and `/api/shortlist/` (`"/shortlists".startsWith("/shortlist")` is true). Staff routes live at `/api/property-shortlists`, a different subtree. Needs a matching CF Access **bypass** app for `crm.nextkey.com.au/shortlist/*` + `/api/shortlist/*`. `isShortlistPortalPath` (its own tiny module, `utils/shortlist-path.ts`, so AppShell doesn't bundle the finance engine) keeps the page chromeless.
- **Masking reuses the partner portal**: `toClientLotView` is built from `toPartnerLot` and picks fields one by one (no referral fee, no stock id); the payload passes `assertNoForbiddenKeys`; the consultant's message and notes and the AI suburb narrative go through `redactSupplierNames`.
- **Photos are streamed, never linked.** `property_media.storage_path` public URLs carry the brochure folder name, which names the estate and supplier (`…/lot_1238_newbridge_spm_revised/facade_05.jpeg`). The page asks `/api/shortlist/<token>/image?item=&n=`; `shortlistImageUrls` only yields our own Supabase public-storage URLs (else the route is an open proxy), facade then gallery, in a stable created_at/id order so index N always resolves to the same file. Floor plans and brochure PDFs are excluded — builder documents.
- **Reports are pure** (`computeReport`): deposit/loan, P&I or IO repayment, stamp duty via `utils/finance/stampDuty.ts` (on the **land** price for a two-part package, else the full price), upfront cash = deposit + duty + `DEFAULT_CLOSING_COSTS`, gross yield, running costs = capacity engine's allowance (max of 25% of rent, 0.9% of value), weekly pre-tax cashflow. The client page imports it and recomputes as the client edits assumptions. Unknown state → duty `null`, no rent → yield/cashflow `null`: the page says "Ask us", never $0. `REPORT_DISCLAIMER` on every page (general information, not credit/financial advice, LMI not included).
- **Rent is usually missing** (43 of 827 active lots on 2026-09-14), so the modal takes an expected rent per property (`rent_weekly_override`); the page labels it "our estimate".
- **Suburb profile** = `getSuburbIntel` (NEXUS) + `getSuburbNarrative` (AI + web, 7-day cache), accepted only for suburbs on that shortlist so a token can't run research on arbitrary places. First load can take ~30s.
- `lot_snapshot` (masked view at send time) keeps an item legible if the stock row is deleted — shown as unavailable. `property_shortlist_events` is append-only (trigger). Invite email is `commercial: true` (unsubscribes suppressed), reply-to the consultant.
...
**Every rendered meeting time goes through `utils/datetime.ts` — never a bare `toLocaleString`.** `"en-AU"` sets the *locale*, not the timezone: an unpinned formatter silently uses whatever runtime it executes in, which is the browser (Brisbane) in a client component but the **server (UTC on Netlify)** in a server component. `/calendar` is a client component and `/appointments` is a server component, so the same booking read 1:00 pm on one page and 3:00 am on the other until both were pinned. `utils/datetime.ts` exports `BUSINESS_TIME_ZONE` (`Australia/Brisbane` — no DST, so a fixed zone is right year-round) plus `formatDateTime` / `formatAgendaDateTime` / `formatTime` / `formatLongDateTime`, and it is pure so both sides can import it. Its tests assert literal strings and run under `TZ=UTC`, so any formatter that regresses to the runtime zone fails the suite.
- **Bucketing by day needs `businessDayKey()`, not `getDate()`** — the day a meeting falls on is a timezone question too, and an 8am Brisbane meeting is the *previous* UTC day.
- **`utils/tasks.ts` is deliberately excluded.** A date-only `due_date` has no instant attached and must be read as *local* midnight; pinning it would make every task due today look overdue until 10am AEST.
- The write path never needed this — `appointments.start_time` is a `timestamptz` UTC instant, and `utils/booking.ts` / `app/api/book/[host]` already pin their own output, which is why confirmation emails were always correct.
...
### Lender Policy database + matching engine (`app/lenders/` + `app/api/lenders/` + `utils/lender-policy/`)
Every Australian home-loan lender's published lending policy, versioned with a source URL and an as-at date behind **every** value, plus a matching engine that takes a client scenario and returns a ranked lender shortlist with per-lender pass/fail reasons and an estimated max loan. Sidebar links **Lender Policy** + **Lender Match** under Command. Full write-up: `docs/FEATURE-lender-policy.md`.

**It is the per-lender parameter layer for `utils/finance/capacity.ts`, not a second calculator.** `estimateMaxLoan()` calls `computeCapacity()` with that lender's own buffer / rent shading / DTI cap / card assessment rate / HEM loading / assessment-rate floor overlaid. `CapacityInputs` grew three OPTIONAL fields for this (`creditCardAssessmentRate`, `hemLoading`, `assessmentFloorRate`); all default to the existing generic behaviour, so every prior caller is unchanged.

- `utils/lender-policy/types.ts` — the taxonomy. Eight sections (servicing, lvr, income, employment, credit, security, product, residency). **Every leaf is a `Fact<T>`** carrying `value` + `asAt` + `sourceUrl` + `confidence` + `status`. Fields are optional throughout: an absent field ("not researched") is a different and honest state from `{value: null, status: 'not_found'}` ("we looked, it isn't published").
- `utils/lender-policy/verify.ts` — **the anti-hallucination gate, and the reason this feature is trustworthy.** The database is written by AI reading lender websites, and the failure mode is not an empty database but a confidently wrong one. `verifyFact()` demotes to `unverified` anything missing a source URL or as-at date, malformed, stale (>2yr), outside `PLAUSIBLE_RANGES` for its field path, or recorded at `low` confidence. `verifyPolicy()` **overwrites the researcher's own `status` claim with the verdict** — a research agent supplies evidence, it does not grant itself clearance. `human_confirmed` is the one status that bypasses the range/confidence heuristics (a broker holding the policy document beats a heuristic) but still requires provenance.
- `utils/lender-policy/match.ts` — the rules engine. Four outcomes per check: `pass` / `fail` / `refer` / **`unknown`**. `unknown` is load-bearing: any check that cannot be evaluated on a *verified* fact returns `unknown`, **never `pass`**, so an unresearched lender is reported as "not enough policy data", never as a fit. A lender with zero real passes can't be `eligible`. Ranking is outcome → score, where score = check outcomes + loan size + data-quality ratio + verified-fact *volume* (ratio alone ties a 1-fact record with a 40-fact one).
- `utils/lender-policy/scenario.ts` — `LenderScenario` **composes** `CapacityInputs` rather than duplicating it, and adds only the policy dimensions (applicant residency/employment basis/tenure/income types, security, credit history, genuine savings).
- `utils/factfind-lender-scenario.ts` — the Fact Find bridge, mirroring `factfind-capacity.ts` (neither `utils/factfind.ts` nor `utils/lender-policy/` knows about the other). **The Fact Find has no field for employment basis, tenure, residency or credit history** — precisely the knock-out fields — so the bridge defaults them and returns an `assumptions[]` list naming every default, which the UI shows *before* the shortlist. Defaulting silently would rebuild the hallucination problem on the input side.
- `utils/lender-policy/store.ts` — Supabase, with the committed research pack as fallback. **A missing table is not an error state**: the library and matcher work from `utils/lender-policy/pack.generated.ts` and the UI says so. What the table buys is *writes*. **Broker field confirmations live in their own table (`lender_policy_field_reviews`) and are folded over the blob on read** — a later research pass rewrites `data` wholesale, so corrections stored inside it would be silently destroyed.
- `utils/lender-policy/source-check.ts` — server-only live URL resolution. A shape check can't catch an invented-but-plausible URL on the right domain, which is exactly what a hallucinating model produces. Includes an SSRF guard: these URLs come from a language model, so loopback/link-local/RFC1918 targets are refused before `fetch`.
- **Research pipeline.** Bulk tranches are Claude Code subagents following `docs/lender-policy-research-brief.md`; `POST /api/lenders/research` is the in-app single-lender refresh and enforces the same contract *in code*: verify → live source check → reject aggregator/comparison-site hosts → only then, and only if `apply` is requested, write. Rejections are returned and logged to `lender_policy_research_runs.rejections`; **if that log is ever empty run after run, the gate has stopped working.**
- Data: `data/lender-policies/<id>.json` is the reviewable source of truth (one file per lender, so a policy change is a git diff); `node scripts/build-lender-pack.mjs` bundles them into `pack.generated.ts`. The JSON is embedded as a *string* parsed at load, not an object literal — 100 deeply-nested records is a lot of pointless inference on every `tsc --noEmit`.
...
- **View vs Download.** `GET /api/shared-folder/[id]` signs an attachment URL by default; `?disposition=inline` signs one *without* `download`, so Storage serves it inline and the client opens it in a new tab. The **View** button only renders for `isViewable()` types (`VIEWABLE_EXTENSIONS` in `utils/shared-folder.ts`), and **the route re-checks that allowlist server-side** — hiding a button is not a control. `.html` / `.htm` / `.svg` are excluded on purpose: Storage serves from `*.supabase.co`, so an inline render executes script on that origin. It can't reach a CRM session (different origin), but it would be a live page hosted on our infrastructure — a phishing surface. Office formats are excluded because browsers download them regardless, and a View button that downloads is a broken promise. The new tab is opened *before* the await, or popup blockers eat it.
...
| `/lenders` | Lender Policy — searchable library of published AU lender policy, with verification state per record |
| `/lenders/[id]` | One lender's full policy, fact by fact, with source links and inline broker confirmation |
| `/lenders/match` | Lender Match — client scenario → ranked shortlist with pass/fail reasons + estimated max loan. Accepts `?factFind=<id>` to prefill from a Fact Find |
...
| `/admin/partners` | **Partners** (staff) — hold-request/deal queue with the supplier behind each lot (grant, decline, advance, release, *release lot details*), and the partner-firm register (onboard, tier, extras, white-label branding, logins). See *Channel-partner portal* |
| `/partner` | **Partner Portal** (PUBLIC, own session, no CRM chrome) — sign-in, then `/partner/stock` (masked stock book + referral fee), `/partner/clients`, `/partner/deals`, `/partner/account` (plan) |
...
- `PARTNER_NOTIFY_EMAILS` (optional — who hears about partner hold requests; default `sean.l@nextkey.com.au`)
- `PARTNER_FEE_FLOOR` / `PARTNER_FEE_PERCENT` (optional — NextKey's share of a lot's gross developer fee; defaults $10,000 / 30%; the floor can't go below $10k)
